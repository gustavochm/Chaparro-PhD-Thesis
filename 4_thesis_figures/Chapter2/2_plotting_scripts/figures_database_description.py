...
def plot_database_description(df_data_fluid, width=3., height=1., marker='.', color_fluid='C0',
                              alpha=0.1, markersize=3.,  bins=25, hmax=4000, linewidth=0.5,
                              edgecolor_fluid='#0A4C86', alpha_grid=0.2, rasterized=True):
    ...
    scatter = ax1.scatter(df_data_fluid['rho*'], df_data_fluid['T*'], marker=marker, color=color_fluid, alpha=alpha, clip_on=True, s=markersize)
    ...
    ax1_histx.hist(df_data_fluid['rho*'], bins=bins, color=color_fluid,
                   linewidth=linewidth, edgecolor=edgecolor_fluid)
    ax1_histy.hist(df_data_fluid['T*'], bins=bins, orientation='horizontal', 
    ...
    scatter2 = ax2.scatter(df_data_fluid['rho*'], 1./df_data_fluid['T*'], marker=marker, color=color_fluid, alpha=alpha, clip_on=True, s=markersize)
    ...
    ax2_histx.hist(df_data_fluid['rho*'], bins=bins, color=color_fluid,
                   linewidth=linewidth, edgecolor=edgecolor_fluid);

    ax2_histy.hist(1./df_data_fluid['T*'], bins=bins, orientation='horizontal',
                   color=color_fluid, linewidth=linewidth, edgecolor=edgecolor_fluid)
    ...
    count_lr = []
    lr_unique = np.unique(df_data_fluid['lr'])
    for lr in lr_unique:
        count_lr.append(np.sum(df_data_fluid['lr'] == lr))  
