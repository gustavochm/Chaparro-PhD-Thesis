...
def plot_brown_curves_second_virial(excel_dict, df_brown, lr_list, width=6., height=4.,
                                    T_lower=0.6, T_upper=10., P_lower=1e-3, P_upper=1e2, B_lower=-10., B_upper=3,
                                    ...
                                    marker_crit='o', marker_zeno='s', marker_boyle='^', marker_charles='v', marker_amagat='o',
    ...
    for lambda_r, ax, axb, plot_label in zip(lr_list, axs, axs_b, plot_labels):
        ...
        # SLE
        df_sle = df_sle[df_sle['P_sle_model'] < 1.1 * P_upper]
        ax.fill_betweenx(df_sle['P_sle_model'], df_sle['T_sle_model'], T_lower, color=color_solid_fill, zorder=0.1, alpha=alpha_solid_fill)
        # ax.plot(df_sle['T_sle_model'], df_sle['P_sle_model'], color=color_solid, zorder=0.1)
