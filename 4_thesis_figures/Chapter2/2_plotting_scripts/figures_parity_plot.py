...
def plot_parity_plot(excel_parity, width=3., height=9., color_train='C0', color_test='C2',
    ...
    property_kwargs['compressibility_factor'] = dict(ax=ax1, lims=[-1, 15], ticks=[0, 5, 10, 15], label=r'$Z$')
    property_kwargs['internal_energy'] = dict(ax=ax2, lims=[-10, 25], ticks=[-10,  0, 10, 20], label=r'$U^*$')
    property_kwargs['B2'] = dict(ax=ax3, lims=[-11, 3], ticks=[-10, -6, -2, 2], label=r'$B_2^*$', plot_index='(c)')
    property_kwargs['isochoric_heat_capacity'] = dict(ax=ax4, lims=[1.4, 3.6], ticks=[1.5, 2.0, 2.5, 3.0, 3.5], label=r'$C_V^*$')
    property_kwargs['rho_isothermal_compressibility'] = dict(ax=ax5, lims=[-0.2, 4.2], ticks=[0., 1., 2., 3., 4.], label=r'$\rho^*\kappa_T^*$')
    property_kwargs['thermal_pressure_coefficient'] = dict(ax=ax6, lims=[-1., 13], ticks=[0., 3., 6., 9., 12.], label=r'$\gamma_V^*$')
