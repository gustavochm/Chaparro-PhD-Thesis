...
def data_brown_curves_solid_lr(fun_dic, lambda_r, lambda_a=6., T_lower=0.5,
    ...
    data_lr = data_phase_equilibria_solid_lr(fun_dic, lambda_r=lambda_r, 
                                             Tlower=T_lower,
                                             initial_triple_point=initial_triple_point, initial_crit_point=initial_crit_point)

    df_info = data_lr['info']
    df_vle = data_lr['vle']
    df_sle = data_lr['sle']
    Tcad_model = df_info['Tcad_model'].values[0]
    T_triple = df_info['T_triple'].values[0]
    T_sle_model = df_sle['T_sle_model'].to_numpy()
    P_sle_model = df_sle['P_sle_model'].to_numpy()
    #######################
    # Virial coefficients #
    #######################
    ...
    # Zeno Curve #
    ##############
    T_zeno_model = np.linspace(T_triple, Tr_upper_zeno*Tboyle_model, n_zeno)[::-1]
    ...
    # deleting points beyond the solid-liquid equilibrium curve
    Psle_inter = interp1d(T_sle_model, P_sle_model, bounds_error=False)
    filter_zeno = pressure_zeno_model < Psle_inter(T_zeno_model)
    T_zeno_model = T_zeno_model[filter_zeno]
    rho_zeno_model = rho_zeno_model[filter_zeno]
    pressure_zeno_model = pressure_zeno_model[filter_zeno]
    ...
    # Charles Curve #
    #################
    T_charles_model = np.linspace(T_triple, Tr_upper_charles*Tcharles_model, n_charles)
    ...
    #deleting possible negative pressures
    final_bool = np.logical_and(pressure_charles_model < 0., bool_Tsat)
    pressure_charles_model[final_bool] = np.nan
    rho_charles_model[np.isnan(pressure_charles_model)] = np.nan
    T_charles_model[np.isnan(pressure_charles_model)] = np.nan
